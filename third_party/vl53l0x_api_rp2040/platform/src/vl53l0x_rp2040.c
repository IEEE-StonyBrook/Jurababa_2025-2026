...
i2c_inst_t* vl53l0x_i2c_port = &i2c0_inst;
uint        vl53l0x_i2c_sda  = 4; // ToF sensors on I2C0: GP4=SDA
uint        vl53l0x_i2c_scl  = 5; // ToF sensors on I2C0: GP5=SCL
...
VL53L0X_Error VL53L0X_dev_i2c_initialise(VL53L0X_Dev_t* pDevice, i2c_inst_t* i2c_port, uint sda,
                                         uint scl, uint16_t i2c_speed_k, uint32_t RangeProfile)
{
    VL53L0X_Error Status;
...
VL53L0X_Error VL53L0X_device_initizlise(VL53L0X_Dev_t* pDevice, uint32_t RangeProfile)
{
    ...
    Status =
        VL53L0X_PerformRefCalibration(pDevice, &VhvSettings, &PhaseCal); // Device Initialization
    if (Status != VL53L0X_ERROR_NONE)
        return Status;
    ...
    Status = VL53L0X_SetLimitCheckEnable(pDevice, VL53L0X_CHECKENABLE_SIGMA_FINAL_RANGE, 1);
    if (Status != VL53L0X_ERROR_NONE)
        return Status;
    ...
    Status = VL53L0X_SetMeasurementTimingBudgetMicroSeconds(pDevice, RangeProfile);
    if (Status != VL53L0X_ERROR_NONE)
        return Status;

    Status = VL53L0X_SetVcselPulsePeriod(pDevice, VL53L0X_VCSEL_PERIOD_PRE_RANGE, 18);
    if (Status != VL53L0X_ERROR_NONE)
        return Status;

    Status = VL53L0X_SetVcselPulsePeriod(pDevice, VL53L0X_VCSEL_PERIOD_FINAL_RANGE, 14);
    if (Status != VL53L0X_ERROR_NONE)
        return Status;
...
int32_t VL53L0X_write_multi(uint8_t address, uint8_t index, uint8_t* pdata, int32_t count)
{
    ...
    if (i2c_write_blocking(vl53l0x_i2c_port, address, i2c_buff, count + 1, false) ==
        PICO_ERROR_GENERIC)
    {
        status = STATUS_FAIL;
    }
...
VL53L0X_Error VL53L0X_SingleRanging(VL53L0X_Dev_t* pDevice, uint16_t* MeasuredData)
{
    ...
    *MeasuredData = 0;
    Status        = VL53L0X_SetDeviceMode(
        pDevice, VL53L0X_DEVICEMODE_SINGLE_RANGING); // Setup in single ranging mode
    if (Status != VL53L0X_ERROR_NONE)
        return Status;

    Status = VL53L0X_PerformSingleRangingMeasurement(pDevice, &RangingMeasurementData);
...
VL53L0X_Error VL53L0X_ContinuousRanging(VL53L0X_Dev_t* pDevice, uint16_t* MeasuredData,
                                        uint16_t RangeCount, uint16_t* validCount)
{
    VL53L0X_Error                    Status = VL53L0X_ERROR_NONE;
    VL53L0X_RangingMeasurementData_t RangingMeasurementData;
