...
Odometry::Odometry(Encoder* leftEncoder, Encoder* rightEncoder, IMU* imu)
    : leftEncoder(leftEncoder), rightEncoder(rightEncoder), imu(imu), lastLeftTicks(0),
      lastRightTicks(0), totalDistanceMM(0.0f), totalAngleDeg(0.0f), deltaDistanceMM(0.0f),
      deltaAngleDeg(0.0f)
{
}
...
void Odometry::update()
{
    ...
    if (deltaAngleDeg > 180.0f)
        deltaAngleDeg -= 360.0f;
    if (deltaAngleDeg < -180.0f)
        deltaAngleDeg += 360.0f;
...
float Odometry::getDistanceMM() const
{
...
float Odometry::getAngleDeg() const
{
...
float Odometry::getDeltaDistanceMM() const
{
    return deltaDistanceMM;
}

// Return rotation change since last update in degrees.
float Odometry::getDeltaAngleDeg() const
{
