                                                         ...
                                                         bool passThroughGoalCells)
{
    ...
    for (std::array<int, 2> endCell : endCells)
    {
        std::vector<MazeNode*> pathToEnd =
            getPathFromCurrPosToCell(internalMouse->getNodeAtPos(endCell[0], endCell[1]),
...
std::vector<MazeNode*> AStarSolver::getPathFromCurrPosToCell(MazeNode* endCell,
                                                             bool      diagMovementAllowed,
                                                             bool      passThroughGoalCells)
{
    ...
    std::vector<std::vector<float>> sCostOfCells(
        ...
        std::vector<float>(internalMouse->getMazeHeight(), std::numeric_limits<float>::infinity()));

    std::priority_queue<AStarNode> toBeProcessedCells;

    sCostOfCells[startNode->getCellXPos()][startNode->getCellYPos()] = 0.0f;
    toBeProcessedCells.push({startNode, 0.0f, getHeuristicDistance(startNode, endCell)});
    ...
    while (!toBeProcessedCells.empty())
    {
        ...
        if (currentNode.node->isProcessed)
            continue;
             ...
             internalMouse->getNodeNeighbors(currentNode.node, diagMovementAllowed))
        {
            ...
            float newS = currentNode.sCost + getHeuristicDistance(currentNode.node, neighbor);
...
std::vector<MazeNode*> AStarSolver::constructPath(MazeNode* startNode, MazeNode* endNode)
{
