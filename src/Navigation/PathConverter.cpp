...
std::string PathConverter::buildLFRPath(MazeNode* startNode, std::array<int, 2> startHeading,
                                        std::vector<MazeNode*> nodePath)
{
    ...
    {
        std::array<int, 2> headingNeeded = {nextNode->getCellXPos() - currNode->getCellXPos(),
                                            nextNode->getCellYPos() - currNode->getCellYPos()};
        std::string addedMoves = calculateMovesNeededBetweenHeadings(currHeading, headingNeeded);
        lfrPath += addedMoves;
        currNode = nextNode;
    }
...
std::string PathConverter::calculateMovesNeededBetweenHeadings(std::array<int, 2> fromHeading,
...
std::string PathConverter::getMovesNeededBy4Cardinal(std::array<int, 2> fromHeading,
                                                     std::array<int, 2> toHeading)
{
    const std::vector<std::array<int, 2>> directionsArray = {{0, 1}, {1, 0}, {0, -1}, {-1, 0}};
                                     ...
                                     std::array<int, 2>              find)
{
    ...
    {
        if (array[0] == find[0] && array[1] == find[1])
            return counter;
        counter++;
    }
    return -1;
