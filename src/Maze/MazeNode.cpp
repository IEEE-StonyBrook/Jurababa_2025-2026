...
void MazeNode::setWallInDirection(char NESWdirection)
{
    switch (tolower(NESWdirection))
    {
        ...
        case 'n':
            addThereIsWall('n');
            if (NCell != nullptr)
                NCell->addThereIsWall('s');
            break;

        case 'e':
            addThereIsWall('e');
            if (ECell != nullptr)
                ECell->addThereIsWall('w');
            break;

        case 's':
            addThereIsWall('s');
            if (SCell != nullptr)
                SCell->addThereIsWall('n');
            break;

        case 'w':
            addThereIsWall('w');
            if (WCell != nullptr)
                WCell->addThereIsWall('e');
            break;
    }
}
...
{
    return n1->getCellXPos() == n2->getCellXPos() and n1->getCellYPos() == n2->getCellYPos();
}

int MazeNode::getCellXPos()
{
    return mazeXPos;
}
int MazeNode::getCellYPos()
{
    return mazeYPos;
}

bool MazeNode::getCellIsExplored()
{
