...
MazeGraph::MazeGraph(int mazeNumRows, int mazeNumCols)
{
    ...
    cellMaze =
        std::vector<std::vector<MazeNode*>>(mazeNumCols, std::vector<MazeNode*>(mazeNumRows));
...
void MazeGraph::linkNeighboringCellsInMaze()
{
    ...
    for (int col = 0; col < colSize; col++)
    {
        for (int row = 0; row < rowSize; row++)
        {
            ...
            if (row > 0)
                currentCell->setCellInDirection(cellMaze[col][row - 1], 'S');
            if (col > 0)
                currentCell->setCellInDirection(cellMaze[col - 1][row], 'W');
        }
    }
}
...
std::vector<MazeNode*> MazeGraph::getNodeNeighbors(MazeNode* node, bool includeDiagNeighbors)
{
    ...
    // Cardinal directions
    if (x > 0)
        neighbors.push_back(cellMaze[x - 1][y]); // West
    if (x < w - 1)
        neighbors.push_back(cellMaze[x + 1][y]); // East
    if (y > 0)
        neighbors.push_back(cellMaze[x][y - 1]); // South
    if (y < h - 1)
        neighbors.push_back(cellMaze[x][y + 1]); // North

    // Diagonal directions
    if (includeDiagNeighbors)
    {
        if (x > 0 && y > 0)
            neighbors.push_back(cellMaze[x - 1][y - 1]); // SW
        if (x > 0 && y < h - 1)
            neighbors.push_back(cellMaze[x - 1][y + 1]); // NW
        if (x < w - 1 && y > 0)
            neighbors.push_back(cellMaze[x + 1][y - 1]); // SE
        if (x < w - 1 && y < h - 1)
...
int MazeGraph::getMazeWidth()
{
    return cellMaze.size();
}

int MazeGraph::getMazeHeight()
{
    return cellMaze[0].size();
}