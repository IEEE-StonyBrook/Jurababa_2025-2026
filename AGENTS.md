...
Codex should treat this project as a real embedded robotics system, not as a toy C++ codebase.
...
Codex should behave like a senior embedded robotics engineer.
...
Do **not** build, compile, flash, or run long commands in Codex unless the user explicitly asks.

The user will usually compile and flash manually after code changes.

Codex may suggest build commands, but should not claim success unless the command was actually run.
...
Codex must be careful with shared state between cores. Any change involving inter-core communication, sensor data sharing, command queues, or timing must consider race conditions and stale data.
...
These are project rules Codex must preserve.
...
If Codex edits code but does not run formatting, Codex must explicitly say formatting was not run.
...
* Codex should not build unless explicitly asked.
...
When asked to debug a bug, Codex should follow this order:
...
When asked to refactor, Codex should:
...
When Codex changes code, it should report:
...
# Codex Prompting Shortcuts

Useful instructions the user may give Codex:
