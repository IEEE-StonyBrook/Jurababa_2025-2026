...
class PathConverter
{
  public:
     ...
     */
    static std::string buildLFR(Cell* start_cell, std::array<int, 2> start_heading,
                                const std::vector<Cell*>& path);

  private:
    static std::string movesForHeadingChange(std::array<int, 2> from, std::array<int, 2> to);
    static std::string cardinalMoves(std::array<int, 2> from, std::array<int, 2> to);
