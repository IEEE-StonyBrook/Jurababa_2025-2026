...
void MotorLabReporter::reportProfile(uint32_t time_ms, float set_position,
                                     float actual_position, float set_speed,
...
void MotorLabReporter::reportController(uint32_t time_ms, float set_position,
                                        float actual_position, float set_speed,
                                        float actual_speed, float control_volts,
...
void MotorLabReporter::reportStep(uint32_t time_ms, float step_voltage,
                                  float speed, float position)
{
    uint32_t elapsed = time_ms - start_time_ms_;
    printf("%lu,%.3f,%.2f,%.2f\n",
           static_cast<unsigned long>(elapsed),
           step_voltage, speed, position);
    sample_count_++;
}
