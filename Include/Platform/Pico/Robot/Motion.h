...
class Motion
{
  public:
    ...
    // ---------------- Forward Motion ----------------
    void startForward(float distance_mm, float top_speed, float final_speed, float accel);
    void forward(float distance_mm, float top_speed, float final_speed, float accel, bool blocking);
    bool isForwardFinished() const;

    // ---------------- Rotational Motion ----------------
    void startTurn(float angle_deg, float omega, float final_omega, float alpha);
    void turn(float angle_deg, float omega, float final_omega, float alpha, bool blocking);
     ...
     */
    void setWallUpdateCallback(std::function<void(bool, bool, bool)> cb) { wallCallback_ = cb; }
